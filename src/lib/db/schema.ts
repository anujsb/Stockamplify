...
export const stockIntraDayPrice = pgTable("stock_intraday_price", {
  ...
  twoHundredDayMovingAverage: decimal("two_hundred_day_moving_average", {
    precision: 18,
    scale: 4,
  }),
...
export const acePortfolioWide = pgTable("ace_portfolio_wide", {
  id: serial("id").primaryKey(),
  company: varchar("company", { length: 255 }).notNull(),
  symbol: varchar("symbol", { length: 20 }),
  c1: numeric("c1", { precision: 6, scale: 3, mode: "number" }),
  c2: numeric("c2", { precision: 6, scale: 3, mode: "number" }),
  c3: numeric("c3", { precision: 6, scale: 3, mode: "number" }),
  c4: numeric("c4", { precision: 6, scale: 3, mode: "number" }),
  c5: numeric("c5", { precision: 6, scale: 3, mode: "number" }),
  valueCr: numeric("value_cr", { precision: 14, scale: 2, mode: "number" }),
  aceId: integer("ace_id").notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

export const aceInvestor = pgTable("ace_investor", {
  aceId: serial("ace_id").primaryKey(),
  aceName: varchar("ace_name", { length: 200 }).notNull(),
  firmName: varchar("firm_name", { length: 200 }),
  isInstitution: boolean("is_institution").notNull().default(false),
  info: text("info"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});

export const aceQuarterColumnMap = pgTable("ace_quarter_column_map", {
  id: serial("id").primaryKey(),
  columnName: varchar("column_name", { length: 2 }).notNull(),
  quarter: date("quarter", { mode: "date" }).notNull(),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
